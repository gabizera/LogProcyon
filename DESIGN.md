...
LogProcyon é uma ferramenta de NOC/retenção de logs. Três pontos específicos
deste projeto, além das regras gerais acima:

1. **Cor funcional contida.** Cor saturada é permitida SOMENTE onde carrega
   função: backend `--ok #2f7d3a` / `--down #b23b3b`; aviso `--warn #b06a1f`;
   protocolo `--tcp #1d6fb8` / `--udp #7a5bb5` / `--icmp #b06a1f` (cor no texto
   + borda translúcida via `--*-tint` / `--*-line`, sem fundo saturado). Badges
   de Tipo NAT (CGNAT/BPA/Estático) permanecem neutros — diferenciados por
   texto. Botões de ação primária são SEMPRE o charcoal com sombra inset
   (`background: var(--ink)`, `color: var(--off-white)`) — nunca gradiente nem
   cor saturada.
...
3. **Fonte.** Camera Plain Variable não está empacotada no projeto; o token
   `--font-text` cai no fallback `ui-sans-serif, system-ui, sans-serif`.
   Alinhamento numérico via `font-variant-numeric: tabular-nums`, nunca fonte
   mono.
